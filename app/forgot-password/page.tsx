...
export default function ForgotPasswordPage() {
  ...
  const [step, setStep] = useState<'main' | 'sent'>('main');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // For now, since we're using local auth without email service,
      // we'll just show a message that password reset isn't available
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 w-full max-w-md border border-white/20">
          <div className="text-center">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">Password Reset Notice</h3>
            ...
            <div className="space-y-3">
              <Link
                href="/login"
                className="block w-full py-2 px-4 bg-emerald-500 text-white rounded-full hover:bg-emerald-600 transition-colors"
              >
                Back to Login
              </Link>
